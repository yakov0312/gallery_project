...
void MemoryAccess::deleteAlbumsOfUser(const int id)
{
	for (auto iter = this->m_albums.begin(); iter != this->m_albums.end();) 
	{
		if (iter->getOwnerId() == id)
		{
			iter = this->m_albums.erase(iter);
		}
		else
		{
			iter++;
...
bool MemoryAccess::doesAlbumExists(const std::string& albumName, int userId) const
{
	for (const auto& album: m_albums) {
...
void MemoryAccess::addPictureToAlbumByName(const std::string& albumName, const Picture& picture, const int ownerId)
{
...
void MemoryAccess::removePictureFromAlbumByName(const std::string& albumName, const std::string& pictureName, const int ownerId) 
{
	auto result = getAlbumIfExists(albumName);
...
void MemoryAccess::untagUserFromAllPictures(const User& user)
{
	for (const auto& album : m_albums)
	{
		if (album.getOwnerId() == user.getId())
		...
		{
			this->untagUserInPicture(album.getName(), picture.getName(), user.getId());
		}
	}
...
int MemoryAccess::createUser(User& user)
{
...
void MemoryAccess::deleteUser(const User& user)
{
	...
	{
		this->deleteAlbumsOfUser(user.getId());
		this->untagUserFromAllPictures(user);
...
bool MemoryAccess::doesUserExists(int userId) const
{
